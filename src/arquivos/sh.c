  ...
  case ' ':
    ...
    if(!access(ecmd->argv[0], F_OK)){
      ...
      if(!access(path, F_OK)){
        ...
        fprintf(stderr, "O comando não foi encontrado");
      }
    }
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
     ...
     * para reportar o erro corretamente. */
     ...
     /*RESPOSTA: O if abaixo executa a mudança de diretório através
      * do comando 'cd'. Caso chdir(buf+3) retorne -1, significa que
      * o diretório não existe */
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "O diretório não existe\n");
      continue;
    }
    /* MARK END task1 */
